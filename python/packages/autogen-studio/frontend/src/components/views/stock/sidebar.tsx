...
interface StockSidebarProps {
    ...
    currentDataPanel: DataPanel | null;
    onToggle: () => void;
    onSelectDataPanel: (dataPanel: DataPanel) => void;
    isLoading?: boolean;
}
...
export const StockSidebar: React.FC<StockSidebarProps> = ({
    isOpen,
    dataPanels: guides,
    currentDataPanel: currentGuide,
    onToggle,
    onSelectDataPanel: onSelectDataPanel,
    isLoading = false,
}) => {
        ...
        <div className="h-full border-r border-secondary">
            ...
            <div className="flex items-center justify-between pt-0 p-4 pl-2 pr-2 border-b border-secondary">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-primary font-medium">Data Panel</span>
            ...
            <div className="overflow-y-auto h-[calc(100%-64px)] mt-4">
               ...
               w-1 bg-opacity-80 rounded ${currentGuide?.id === guide.id ? "bg-accent" : "bg-tertiary"
                                }`}
                        />
                        <div
                            className={`group ml-1 flex flex-col p-2 rounded-l cursor-pointer hover:bg-secondary ${currentGuide?.id === guide.id
                                ? "border-accent bg-secondary"
                                : "border-transparent"
                                }`}
                            onClick={() => onSelectDataPanel(guide)}
                        >
                            {/* Guide Title */}
